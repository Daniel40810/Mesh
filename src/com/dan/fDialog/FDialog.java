...
public class FDialog extends JDialog {
    ...
    /** Optionaler Akzent-Rahmen entlang der gerundeten Inhaltskante (null = keiner). */
    private Color contentBorderColor = null;
    ...
    /** true, sobald das FDialog-Grundgerüst (Taskbar + componentPane) steht. */
    ...
    /** Komfort-Konstruktor: modal=true → APPLICATION_MODAL, false → MODELESS. */
    public FDialog(Window owner, String title, boolean modal) {
    ...
    private void initUI() {
        ...
        componentPane = new JPanel() {
            ...
            @Override protected void paintChildren(Graphics g) {
                ...
                // Akzent-Rahmen entlang exakt derselben Rundung wie Füllung/Clip
                if (contentBorderColor != null) {
                    Graphics2D gb = (Graphics2D) g.create();
                    gb.setRenderingHint(RenderingHints.KEY_ANTIALIASING,
                            RenderingHints.VALUE_ANTIALIAS_ON);
                    gb.setColor(contentBorderColor);
                    gb.setStroke(new BasicStroke(1f));
                    gb.draw(new RoundRectangle2D.Float(
                            0.5f, -ARC, getWidth() - 1f, getHeight() + ARC - 1f,
                            ARC * 2, ARC * 2));
                    gb.dispose();
                }
            }
    ...
    /**
     * Umgeleitet: Nach dem Aufbau des Grundgerüsts wird jeder
     * {@code setContentPane(...)}-Aufruf (auch aus Unterklassen) in den
     * {@link #getComponentPane() componentPane} <b>unter</b> die Taskbar
     * gehängt, statt das Grundgerüst mit Taskbar und gerundetem Rahmen zu
     * ersetzen. Dadurch bleibt die Taskbar bei allen FDialog-Ableitungen
     * erhalten, ohne dass diese ihren Code ändern müssen.
     */
    @Override
    public void setContentPane(Container contentPane) {
        if (!uiReady || componentPane == null) {
            super.setContentPane(contentPane);
            return;
        }
        componentPane.removeAll();
        componentPane.add(contentPane, BorderLayout.CENTER);
    ...
    /**
     * Eckradius des gerundeten Inhaltsbereichs. Praktisch für einen
     * {@link FRoundedBorder} auf aufliegenden Panels, damit deren Rahmen der
     * Rundung folgt (unten rund, oben eckig).
     */
    public int getContentArc() {
        return ARC;
    }

    /**
     * Setzt einen Akzent-Rahmen, der exakt der gerundeten Unterkante des
     * Inhaltsbereichs folgt (oben eckig am Taskbar-Anschluss). {@code null}
     * schaltet ihn ab. Damit müssen aufliegende Panels keinen eigenen
     * (rechteckigen) Rahmen mehr setzen.
     */
    public void setContentBorderColor(Color c) {
        this.contentBorderColor = c;
